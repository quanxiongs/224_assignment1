import os
os.getcwd()
os.chdir('E:\\cs224d\\assignment1\\assignment1')
...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    sqsum = np.sum((x.T)**2,axis=0)
    x = (x.T / np.sqrt(sqsum)).T
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    grad = dx.reshape((N,1))*r.reshpe((1,D)) #broadcasted
    ...
    #np.array([[1,2,3,4,5]]).T*np.array([[2,3,4]])
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    labels = np.array([1] + [-1 for k in range(10)])
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    c_idx = tokens[currentWord]  #dictionary with indices
...
def test_word2vec():
    ...
    gradcheck_naive(lambda vec: word2vec_sgd_wrapper(skipgram, dummy_tokens, 
...
help(type('dummy',(),{})())
help(type)
word2vec_sgd_wrapper(skipgram,dict([("a",0), ("b",1), ("c",2),("d",3),("e",4)]),
                                   normalizeRows(np.random.randn(10,3)),dataset,5)
"""
